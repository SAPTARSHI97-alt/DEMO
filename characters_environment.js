...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press play button to start the game and use your nose to play :) ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(noseX != undefined && gameConfig.status==='start' && game_status=='start' ) {
    document.getElementById('status').innerHTML = "whatever you want write here"
